...
class FeedbackCollectionSystemImplementation(FeedbackCollectionSystem):
    ...
    @classmethod
    def update_question(self, index, new_question):
        if 0 <= index < len(Questionnaire.questions):
            Questionnaire.questions[index] = new_question
            return True
        else:
            return False

    # def collect_project_feedback1():
    ...
    #             if not FeedbackCollectionSystemImplementation.update_question(i, new_question):
                ...
                if admin_username == 'admin' and pwd == feedback_system.admin_password:
                    while True:
                        ...
                        if opt == "1":
                            ...
                            feed = collect_project_feedback1()
                            index=int(input("Enter index whic question should be updated:"))
                            new_question=input("enter a updated question")
                            if feedback_system.update_question(index, new_question):
                                print(f"Successfully updated the feedback for {index}")
                            else:
                                print(f"Failed to update the feedback for {index}")
                        elif opt == "3":
